...
void connectMQTT() {
  if (mqttClient.connect("ESP8266Client", mqttUsername, mqttPassword)) {
    ...
    logMessage += "Conexión exitosa al broker MQTT\n";
    Serial.println("Conectado al broker MQTT");
  } else {
    mqttConnected = false;
    reconnectAttempts++;
    logMessage += "Error de conexión al broker. Intento " + String(reconnectAttempts) + "\n";
...
void handleRoot() {
  ...
  html += "<p>Estado del semáforo: " + color + "</p>";
  html += "<p>Humedad: " + humedad + "</p>";
  html += "<h2>Logs:</h2>";
  html += "<pre>" + logMessage + "</pre>";
  html += "</body></html>";
